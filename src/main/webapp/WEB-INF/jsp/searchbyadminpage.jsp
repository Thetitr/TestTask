...
<c:if test="${ not empty users}">
  ...
  <c:if test="${admin=='false'}">
    <h1>Users without admin rights</h1>
  </c:if>
</c:if>
...
<c:if test="${not empty users}">
  ...
  <br>
  <c:url var="previousUrl" value="/main/search/admin?page=${param.page-1}&admin=${param.admin}" />
  <c:url var="nextUrl" value="/main/search/admin?page=${param.page+1}&admin=${param.admin}" />
...
<c:if test="${empty users}">
  <c:if test="${admin=='true'}">
    <p>Admins aren't found</p>
  </c:if>
  <c:if test="${admin=='false'}">
    <p>Users without admin rights aren't found</p>
  </c:if>
</c:if>
