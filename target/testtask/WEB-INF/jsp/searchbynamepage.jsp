...
<c:if test="${not empty users}">
  <h1>Users with name like ${namepattern}</h1>
</c:if>
...
<c:if test="${not empty users}">
  ...
  <br>
  <c:url var="previousUrl" value="/main/search/name?page=${param.page-1}&name=${param.name}" />
  <c:url var="nextUrl" value="/main/search/name?page=${param.page+1}&name=${param.name}" />
...
<c:if test="${empty users}">
  Users with this name aren't found
</c:if>
